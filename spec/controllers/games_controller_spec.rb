...
RSpec.describe GamesController, type: :controller do
  ...
  context 'Usual user' do
    ...
    it 'uses 50/50 help' do
      ...
      expect(ff).to be
      expect(ff).to include(game.current_game_question.correct_answer_key)
      expect(%w(a b c d) - [game.current_game_question.correct_answer_key])
          .to include((ff - [game.current_game_question.correct_answer_key])[0])
      expect(response).to redirect_to(game_path(game))
    end

    it 'uses friend help' do
      # сперва проверяем что в подсказках текущего вопроса пусто
